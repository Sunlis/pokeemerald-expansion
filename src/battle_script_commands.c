...
static void Cmd_adjustdamage(void)
{
    ...
    for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (GetBattlerAbility(battlerDef) == ABILITY_ICE_FACE && (IsBattleMovePhysical(gCurrentMove) || IsBattleMoveSpecial(gCurrentMove)) && gBattleMons[battlerDef].species == SPECIES_EISCUE)
        {
...
static bool32 TryCheekPouch(u32 battler, u32 itemId)
{
        ...
        && !IsBattlerAtMaxHp(battler))
    {
        ...
        gBattleStruct->moveDamage[battler] = MaybeLowerHealingForPoison(battler, gBattleStruct->moveDamage[battler]);
        if (gBattleStruct->moveDamage[battler] == 0)
            gBattleStruct->moveDamage[battler] = 1;
        gBattleStruct->moveDamage[battler] *= -1;
        gBattlerAbility = battler;
...
static void Cmd_trysetrest(void)
{
    ...
    const u8 *failInstr = cmd->failInstr;
    gBattlerTarget = gBattlerAttacker;
    // gBattleStruct->moveDamage[gBattlerTarget] = gBattleMons[gBattlerTarget].maxHP * (-1); redundant

    gBattleStruct->moveDamage[gBattlerTarget] = MaybeLowerHealingForPoison(
        gBattlerTarget,
    ...
    else
    {
        ...
        else
        {
            ...
            if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
                gBattleStruct->moveDamage[gBattlerAttacker] = 1;
            gBattleStruct->moveDamage[gBattlerAttacker] *= -1;
            gBattleStruct->moveDamage[gBattlerAttacker] = MaybeLowerHealingForPoison(battler, gBattleStruct->moveDamage[gBattlerAttacker]);
            gBattlescriptCurrInstr = cmd->nextInstr;
            gBattlerTarget = gBattlerAttacker;
        }
    }
...
static void Cmd_switchoutabilities(void)
{
    ...
    else
    {
        ...
        case ABILITY_REGENERATOR:
        {
            ...
            regenerate = MaybeLowerHealingForPoison(battler, regenerate);
...
void BS_TryHealPulse(void)  //bulagHP sean this script is called for several move types like pollen puff, heal pulse ("EFFECT_")
{
    ...
    else
    {
        ...
        gBattleStruct->moveDamage[gBattlerTarget] = MaybeLowerHealingForPoison(battler, gBattleStruct->moveDamage[gBattlerTarget]);
        if (gBattleStruct->moveDamage[gBattlerTarget] == 0)
            gBattleStruct->moveDamage[gBattlerTarget] = -1;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
